...
class CodeAnalyzer:
    def __init__(self, project_path: str):
        self.project_path = project_path # đường dẫn đến thư mục dự án
        self.metrics = {
            'loc': 0, # tổng số dòng mã
            'cyclomatic_complexity': 0, # độ phức tạp cyclomatic
            'code_duplication': 0.0, # tỷ lệ mã trùng lặp
            'cohesion': 0.0, # mức độ kết nối của các lớp
            'coupling': 0.0 # mức độ phụ thuộc giữa các module
        }
        self.files_analyzed = [] # danh sách các tệp đã phân tích
        self.duplicate_lines = [] # danh sách các dòng mã trùng lặp
        self.class_methods = defaultdict(list) # danh sách các phương thức trong lớp
    ...
    def count_loc(self, file_path: str) -> int:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                loc = sum(1 for line in lines if line.strip() and not line.strip().startswith('#')) # đếm số dòng không phải là bình luận và không phải là dòng trống
                return loc
    ...
    def calculate_cyclomatic_complexity(self, file_path: str) -> int:
        ...
        '''Tính độ phức tạp cyclomatic(số đường đi độc lập trong mã) bằng cách sử dụng AST (Abstract Syntax Tree).'''
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                tree = ast.parse(file.read()) # phân tích cú pháp mã nguồn thành cây cú pháp trừu tượng
            
            complexity = 1
            for node in ast.walk(tree): # duyệt qua tất cả các nút trong cây cú pháp
                if isinstance(node, (ast.If, ast.For, ast.While, ast.Try, ast.With)): # mỗi cấu trúc điểu khiển(if, for, while, try, with)
                    complexity += 1 # tăng độ phức tạp lên 1
                elif isinstance(node, ast.BoolOp): # nếu là toán tử logic (and, or)
